...
class Modula_Image {
    ...
    public function get_image_info( $args ) {
        ...
        if ( empty( $url ) ) {
            return new WP_Error( 'modula-gallery-error-no-url', __( 'No image URL specified for cropping.', 'modula-gallery' ) );
        }
        ...
        if ( ! file_exists( $file_path ) ) {
            return new WP_Error( 'modula-gallery-error-no-file', __( 'No file could be found for the image URL specified.', 'modula-gallery' ) );
        }
        ...
        if ( ! $size ) {
            return new WP_Error( 'modula-gallery-error-no-size', __( 'The dimensions of the original image could not be retrieved for cropping.', 'modula-gallery' ) );
        }
    ...
    public function resize_image( $url, $width = null, $height = null, $crop = false, $align = 'c', $quality = 100, $retina = false, $data = array(), $force_overwrite = false ) {
        ...
        if ( ! file_exists( $dest_file_name ) || ( file_exists( $dest_file_name ) && $force_overwrite ) ) {
            ...
            if ( is_wp_error( $editor ) ) {
                return new WP_Error( 'modula-gallery-error-no-editor', __( 'No image editor could be selected. Please verify with your webhost that you have either the GD or Imagick image library compiled with your PHP install on your server.', 'modula-gallery' ) );
            }
